...
Otevřete složku s projektem ve VS Code.
Nainstalujte rozšíření Live Server (od Ritwick Dey).
...
Nahrajte tento projekt do svého Git repozitáře (GitHub / GitLab / Bitbucket).
Přihlaste se do administračního panelu Netlify (https://app.netlify.com/).
Klikněte na Add new site -> Import an existing project.
Vyberte svůj repozitář.
Nastavení buildu (Build settings):
...
Klikněte na Deploy site. Od této chvíle se při každém pushnutí do Git větve web automaticky zaktualizuje.
Ruční deploy (Drag & Drop)
Pokud nepoužíváte Git:
Přejděte na https://app.netlify.com/.
Přetáhněte celou složku s projektem (obsahující index.html, složku foto, netlify.toml a README.md) do vyznačeného Drag & Drop boxu na Netlify.
